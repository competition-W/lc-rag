...
class DashScopeVLM:
                ...
                if isinstance(block, TextBlock):
                    ...
                    if block.path:
                        ...
                        try:
                            ...
                            # 如果解码失败，假设它可能已经是一个路径
                            content.append({"image": self._prepare_image_path(block.image.decode('utf-8'))})
    ...
    async def acomplete(self, prompt: str, images: Union[str, List[str], List[ImageDocument], ImageDocument], system_prompt: Optional[str] = None, **kwargs) -> str:
        """异步调用模型获取回复 (目前仍为同步实现)"""
        return self.complete(prompt, images, system_prompt, **kwargs)
    ...
    async def astream_complete(self, prompt: str, images: Union[str, List[str], List[ImageDocument], ImageDocument], system_prompt: Optional[str] = None, **kwargs) -> AsyncGenerator[str, None]:
        """异步流式调用模型获取回复 (目前仍为同步实现)"""
        for chunk in self.stream_complete(prompt, images, system_prompt, **kwargs):
            yield chunk
...
# import os
# from typing import Optional, Dict, List, Union, Generator, AsyncGenerator
# from dashscope import MultiModalConversation
# from pathlib import Path
# from llama_index.core.schema import ImageDocument
...
#     def complete(self, prompt: str, images: Union[str, List[str], List[ImageDocument], ImageDocument], system_prompt: Optional[str] = None) -> str:
#         """同步调用模型获取回复，支持多种图像输入类型"""
#         image_paths = self._process_input_images(images)
#         messages = self._prepare_messages(prompt, image_paths, system_prompt)
#         params = self._prepare_parameters(stream=False)
...
#     def stream_complete(self, prompt: str, images: Union[str, List[str], List[ImageDocument], ImageDocument], system_prompt: Optional[str] = None) -> Generator[str, None, None]:
#         """流式调用模型获取回复，支持多种图像输入类型"""
#         image_paths = self._process_input_images(images)
#         messages = self._prepare_messages(prompt, image_paths, system_prompt)
#         params = self._prepare_parameters(stream=True)
...
#     async def acomplete(self, prompt: str, images: Union[str, List[str], List[ImageDocument], ImageDocument], system_prompt: Optional[str] = None) -> str:
#         """异步调用模型获取回复 (目前仍为同步实现)"""
#         return self.complete(prompt, images, system_prompt)
    
#     async def astream_complete(self, prompt: str, images: Union[str, List[str], List[ImageDocument], ImageDocument], system_prompt: Optional[str] = None) -> AsyncGenerator[str, None]:
#         """异步流式调用模型获取回复 (目前仍为同步实现)"""
#         for chunk in self.stream_complete(prompt, images, system_prompt):
#             yield chunk
